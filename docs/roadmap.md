...
- S5: `vp check`, `vp test`, and `vp run -r build` green; measured initial JS <250KB gzip; Lighthouse CI passes; no PBDB-direct browser calls; zero map dots (labels + leader lines only).
...
**Globe labels + leader lines (locked Q8 — replaces all map dots/markers):**

- Zoomed out: country name labels only (MapLibre symbol layer from bundled polygons). Small countries hide labels until zoom ≥ threshold where label fits safely (collision-tested, no overlap).
- On CountryFocus: camera centers selected country; `moss` 1.5px leader lines fan from each findspot to a small `card` callout chip (taxon short name, max 8 visible, rest grouped "+N more"); lines redraw on era change; no dots anywhere.
- Leader geometry (exact): line runs findspot → chip anchor on the globe-panel edge nearest the chip's drawer column; chips stack vertically in a fixed right-edge rail (8 slots, `card` bg, 1px `line` border, 8px gap), never float over the globe. Grouping = by `tid` count desc; overflow → "+N more" chip at rail bottom. Chips are buttons → open same modal as cards.
...
- F7 labels: zoomed-out globe shows country labels, small ones hidden until safe zoom; no dots at any zoom.
...
| `leaders` (replaces `sites` markers — locked Q8) | `leaders(cards, max): Leader[]`, `render(leaders, layout): void`                                          | planned `apps/website/src/globe/leaders.ts`, `packages/utils/src/leaders.ts` + test                                               | no visible point layer; flatten `TaxonCard.sites`; deterministic group/count order; single GeoJSON line layer (`moss` 1.5px) from findspot to a screen-derived rail anchor; max 8 chips + "+N more"; update on map move, resize, and era change; cap 500 segments | vitest: grouping, cap, overflow, deterministic order; visual: redraw after pan/resize/era                                    |
...
#### T2.3 `feat(globe): leader lines (no dots)` — Blocked by: T2.2, T1.2b.

Mentor: `mentor: leader lines?` Files: planned `apps/website/src/globe/leaders.ts`, `packages/utils/src/leaders.ts` + test. DoD: CountryFocus shows ≤8 callouts + lines, zero dots; redraws on era change (locked Q8). Check: `vp test` + visual.
...
- M2.3.2 `render` single GeoJSON line layer (`moss` 1.5px) + callout chips (`card` bg, Inter 500); derive line endpoints from the chip rail's screen anchors and update on map move/zoom/resize; "+N more" chip when grouped; clear on era change. On narrow screens use a bottom rail inside the globe panel so the geometry remains defined.
...
- M5.3.1 F1+F2+F7: first-visit + era switch + back-button + label-only zoomed-out check (zero dots).
...
| T2.3  | feat(globe): leader lines, no dots (cap 8 callouts)         | ready-for-agent | T2.2, T1.2b                 | single layer         | `vp test`, benchmark                            |
...
Non-goals (locked 2026-09-06): auth, CMS, uploads, native mobile, paleogeographic reconstruction, i18n, analytics, PWA, paid tiles, map dots (labels + leader lines only). Static frontend + serverless proxy. Risk: PBDB schema drift → pin `data1.2`, validate in proxy, fail CI loudly. Risk: PBDB slowness/429 → client fallback plus bounded proxy retries (§2.3). Risk: proxy cache semantics → implement only the stale behavior the chosen cache can prove (§3.5, D6). Risk: silhouette licensing → own art only (ADR-003). Risk: leader-line overload → cap 8 callouts + grouping (§2.1). Risk: scope creep → any new ask needs a `gh` issue + ADR before code.
...
✅ Locked: NO dots anywhere. Zoomed-out = country labels only (small hidden until safe zoom, F7). CountryFocus = camera centers country + `moss` 1.5px leader lines to ≤8 callout chips + "+N more". `sites/cluster` module replaced by `leaders` (§2.1, T2.3, quirks 19–20).
